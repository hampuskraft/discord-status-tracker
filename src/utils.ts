...
export async function editWebhookMessage(
	...
	if (!response.ok) {
		throw new Error(
			`Failed to edit message: ${response.status} ${response.statusText}`,
		);
	}
	return response.json();
}
...
const RESOLVED_STATUSES = new Set([
	StatusPageIncidentStatus.Resolved,
	StatusPageIncidentStatus.Postmortem,
]);
...
export async function updateIncident(
	...
	const message = messageId
		? await editWebhookMessage(env, messageId, embed)
		: await sendWebhookMessage(env, embed);
...
export async function checkForUpdates(env: Env): Promise<void> {
	const { incidents } = await fetch(`${API_BASE}/incidents.json`).then((r) =>
		r.json<StatusPageResult>(),
	);
